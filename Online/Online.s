...
.set ODB_IS_WARMUP, ODB_DESYNC_RECOVERY_FIGHTER_ARR + DFRE_SIZE * 4 # u8, 1 = warmup CPU match
.set ODB_SIZE, ODB_IS_WARMUP + 1
...
.set MSRB_IS_WARMUP, MSRB_ALT_STAGE_MODE + 1 # u8, 1 = warmup CPU match active
.set MSRB_SIZE, MSRB_IS_WARMUP + 1
...
.set CSSDT_WARMUP_CPU_CHAR, CSSDT_TEAM_COSTUME_IDX + 1 # u8, CPU character ID (0-25), default Fox=2
.set CSSDT_WARMUP_CPU_LEVEL, CSSDT_WARMUP_CPU_CHAR + 1 # u8, CPU level (1-9), default 9
.set CSSDT_WARMUP_STAGE_IDX, CSSDT_WARMUP_CPU_LEVEL + 1 # u8, index into stage list (0-5)
.set CSSDT_SIZE, CSSDT_WARMUP_STAGE_IDX + 1
